...
const EXPERIMENT_CONFIG = {
  ...
  scaleDefinitions: {
    ...
    emotionScale: { text: "Emotion Distribution", type: "slider" },
...
async function buildExperimentTrials(config, genderCondition) {
  ...
  // Map videos with new column names and emotionCode
  const maleVideos = maleRows.map((row, idx) => ({
    id: `M${idx + 1}`,
    url: row.videoFile,
    gender: "M",
    emotion: row.emotionCode
  })).filter(v => v.url);
  ...
  const emotionAVideos = selectedVideos.filter(v => v.emotion === "A");
  const emotionHVideos = selectedVideos.filter(v => v.emotion === "H");
  const emotionSVideos = selectedVideos.filter(v => v.emotion === "S");

  if (!emotionAVideos.length) throw new Error(`No angry videos found for gender ${genderCondition}`);
  if (!emotionHVideos.length) throw new Error(`No happy videos found for gender ${genderCondition}`);
  if (!emotionSVideos.length) throw new Error(`No sad videos found for gender ${genderCondition}`);
  ...
  const ratioConditions = conditionRows
    ...
    .map((row, index) => ({
      ...
      numEmotion1: Number(row.num_emotion1),
      numEmotion2: Number(row.num_emotion2),
      ratioLabel: `${row.num_emotion1}:${row.num_emotion2}`,
      emotionPairLabel: `${row.emotion1}:${row.emotion2}`,
      sourceRow: row
    }));
  ...
  const baseVideoSets = ratioConditions.map(condition => {
    const emotion1Videos = condition.emotion1 === "A" ? emotionAVideos : condition.emotion1 === "H" ? emotionHVideos : emotionSVideos;
    const emotion2Videos = condition.emotion2 === "A" ? emotionAVideos : condition.emotion2 === "H" ? emotionHVideos : emotionSVideos;
    const emotion1Set = sampleWithoutReplacementIfPossible(emotion1Videos, condition.numEmotion1);
    const emotion2Set = sampleWithoutReplacementIfPossible(emotion2Videos, condition.numEmotion2);
    return shuffleArray([...emotion1Set, ...emotion2Set]);
  });
  ...
  for (const blockSpec of finalBlockOrder) {
    ...
    for (const condition of randomizedConditions) {
      for (let repetition = 0; repetition < 5; repetition++) {
        ...
        trials.push({
          trialId: `B${blockSpec.blockId}_C${condition.conditionIndex + 1}_R${repetition + 1}_${condition.emotionPairLabel}_${condition.ratioLabel}`,
          meta: {
            ...
            emotion1: condition.emotion1,
            emotion2: condition.emotion2,
            numEmotion1: condition.numEmotion1,
            numEmotion2: condition.numEmotion2,
            emotionPairLabel: condition.emotionPairLabel,
  ...
  return {
    ...
    buildInfo: {
      ...
      emotionAVideoCount: emotionAVideos.length,
      emotionHVideoCount: emotionHVideos.length,
      emotionSVideoCount: emotionSVideos.length,
...
class ExperimentController {
  ...
  _wireEvents() {
    ...
    // Emotion slider event listeners
    document.getElementById("emotion-submit-btn").addEventListener("click", () => {
      this.handleEmotionSubmit().catch(err => alert(`Experiment error: ${err.message || err}`));
    });
  ...
  showCurrentQuestion() {
    ...
    // Check if this is an emotion scale question
    const isEmotionScale = q.scaleKey === "emotionScale";
    const emotionSliderContainer = document.getElementById("emotion-slider-container");

    if (isEmotionScale) {
      // Show emotion slider UI
      this.questionImage.style.display = "none";
      emotionSliderContainer.classList.remove("hidden");
      this.setupEmotionSliders();
      this.currentQuestionStartPerf = nowMs();
      this.acceptingQuestionInput = true;
      return;
    } else {
      // Hide emotion slider UI
      emotionSliderContainer.classList.add("hidden");
    }
  ...
  async handleQuestionKeypress(event) {
    ...
    // Skip keyboard input for emotion scale (uses sliders)
    if (q.scaleKey === "emotionScale") return;
  ...
  setupEmotionSliders() {
    const angrySlider = document.getElementById("angry-slider");
    const happySlider = document.getElementById("happy-slider");
    const sadSlider = document.getElementById("sad-slider");
    const angryValue = document.getElementById("angry-value");
    const happyValue = document.getElementById("happy-value");
    const sadValue = document.getElementById("sad-value");
    const totalDisplay = document.getElementById("emotion-total");
    const errorDisplay = document.getElementById("emotion-error");
    const submitBtn = document.getElementById("emotion-submit-btn");

    // Reset sliders
    angrySlider.value = 0;
    happySlider.value = 0;
    sadSlider.value = 0;
    angryValue.textContent = "0";
    ...
    const updateTotal = () => {
      const angry = parseInt(angrySlider.value);
      const happy = parseInt(happySlider.value);
      const sad = parseInt(sadSlider.value);
      const total = angry + happy + sad;

      angryValue.textContent = angry;
    ...
    angrySlider.addEventListener("input", updateTotal);
    happySlider.addEventListener("input", updateTotal);
    sadSlider.addEventListener("input", updateTotal);
  }

  async handleEmotionSubmit() {
    ...
    // Add three separate response entries, one for each emotion
    const emotions = [
    ...
    for (const emotion of emotions) {
      this.currentTrialResponses.push({
        questionIndex: this.currentQuestionIndex,
        scaleKey: `emotionScale_${emotion.name}`,
        questionText: `Emotion: ${emotion.name}`,
        questionImage: "",
        min: 0,
        max: 100,
        step: 25,
  ...
  async finishExperiment() {
    ...
    if (this.config.saveToGoogleDrive) {
      try {
        ...
        setTimeout(() => {
          window.location.href = "https://app.prolific.com/submissions/complete?cc=COL64ZSQ";
        }, 3000);
      ...
      setTimeout(() => {
        window.location.href = "https://app.prolific.com/submissions/complete?cc=C1HIP74G";
      }, 3000);
    }
  }
  ...
  makeCSVText() {
    ...
    const headers = [
      ...
      "genderCondition",
      "trialIndex", "withinBlockTrialIndex", "trialId", "blockId", "blockOrder", "conditionIndex", "emotionPairLabel", "ratioLabel", "emotion1", "emotion2", "numEmotion1", "numEmotion2",
    ...
    for (const t of this.data.trials) {
      ...
      const videoOrder = (t.videoOrder || [])
        .map(v => `${v.slotIndex + 1}:${v.gender}:${v.emotion}:${v.url}`)
        .join("|");
